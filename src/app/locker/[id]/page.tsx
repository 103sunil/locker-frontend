...
export default function Locker(props: { params: Promise<{ id: string }> }) {
  ...
  const uploadFile = async () => {
    ...
    if (initResponse.success) {
      const { url, s3Key } = initResponse.data as { url: string; s3Key: string };
      ...
      toast.error(initResponse.error || "Failed to initialize upload");
      setLoading(false);
      setUploadProgress(0);
    }
  };
            ...
            {image.get("file") ? (
              <span className="w-full">
                <button
                  ...
                  className={`${loading ? "bg-grey cursor-not-allowed" : "bg-black"
